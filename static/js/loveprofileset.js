            ...
            try{
                ...
                const alreadysubmit = (await getDoc(doc(db, "loveSubmitdata", uid))).data() ?? undefined;
                ...
                document.getElementById("subbutton").addEventListener('click',async(e)=>{
                    ...
                    const usersdoc = doc(db, "userprofile", uid);
                    // userprofile 문서를 가져옵니다.
                    const updoc = await getDoc(usersdoc);
                    const {age,
                        gender,
                        major,
                        smoke,
                    ...
                    const loveProfile = doc(db, "loveSubmitdata",uid);
                    ...
                    const loveProfileDocRef = doc(db, "loveSubmit", gender + "사용자", wishgender[0], uid);
                    ...
                    const userProfileDocRef = doc(db, "loveSubmit", gender + "사용자", "userprofile", uid);
                    ...
                    window.location.href='/lmatching-finish'
                });
            }catch(err){
                alert("다시 설정해주세요.")
                window.location.href='/lovematching'
            }